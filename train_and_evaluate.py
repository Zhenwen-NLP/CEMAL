...
def train_tree(config, input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
	...
	embedded = None
	orig_idx = None
	if config.embedding == 'bert' or config.embedding == 'roberta' or config.embedding == 'deberta':
		contextual_input = index_batch_to_words(input_batch, input_length, input_lang)
		input_seq1, input_len1, num_pos_new = embedding(contextual_input)
		#print(input_seq1)
	...
	copy_num_len = [len(_) for _ in num_pos_new]
	...
	all_nums_encoder_outputs = get_all_number_encoder_outputs(encoder_outputs, num_pos_new, batch_size, num_size,
															  encoder.hidden_size)
